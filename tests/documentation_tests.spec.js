...
test.describe('Boost Documentation Tests', () => {
  ...
  test('Documentation page loads with table of contents', async ({ page }, testInfo) => {
    ...
    for (const selector of tocSelectors) {
      ...
      if (count > 0) {
        ...
        if (isVisible) {
          ...
          fs.appendFileSync('test-logs.txt', `${testId} Table of contents found\n`);
          tocFound = true;
          break;
        }
      }
    ...
    if (tocFound) {
      fs.appendFileSync('test-logs.txt', `${testId} Documentation TOC verified\n`);
    } else {
      fs.appendFileSync('test-logs.txt', `${testId} No explicit TOC found - doc may use different navigation\n`);
    }
  ...
  test('Library documentation links are accessible', async ({ page }, testInfo) => {
    ...
    // Find library links
    const popularLibraries = ['asio', 'filesystem', 'algorithm', 'thread', 'beast'];
    ...
    for (const libName of popularLibraries) {
      ...
      if (count > 0) {
        ...
        if (isVisible) {
          ...
          fs.appendFileSync('test-logs.txt', `${testId} Found ${libName} library: ${href}\n`);
          ...
          if (foundLibraries === 1) {
            ...
            fs.appendFileSync('test-logs.txt', `${testId} Navigated to library doc: ${newUrl}\n`);
    ...
    fs.appendFileSync('test-logs.txt', `${testId} Found ${foundLibraries} library links\n`);
  });
  ...
  test('Documentation breadcrumbs navigation works', async ({ page }, testInfo) => {
    ...
    for (const selector of breadcrumbSelectors) {
      ...
      if (count > 0) {
        ...
        if (isVisible) {
          fs.appendFileSync('test-logs.txt', `${testId} Breadcrumbs found\n`);
          ...
          if (breadcrumbLinks.length > 0) {
            ...
            if (href && href !== '#') {
              ...
              fs.appendFileSync('test-logs.txt', `${testId} Breadcrumb navigation: ${urlBefore} -> ${urlAfter}\n`);
    ...
    if (breadcrumbsFound) {
      fs.appendFileSync('test-logs.txt', `${testId} Breadcrumbs navigation verified\n`);
    } else {
      fs.appendFileSync('test-logs.txt', `${testId} No breadcrumbs found - may not be used on this page\n`);
    }
  });
  ...
  test('Documentation version switcher works', async ({ page }, testInfo) => {
    ...
    if (versionSwitcher) {
      ...
      if (tagName === 'select') {
        ...
        fs.appendFileSync('test-logs.txt', `${testId} Found ${optionCount} version options\n`);
        ...
        if (optionCount > 1) {
          ...
          fs.appendFileSync('test-logs.txt', `${testId} Version switch: ${urlBefore} -> ${urlAfter}\n`);
        }
      }
    } else {
      fs.appendFileSync('test-logs.txt', `${testId} No version switcher found - may be on latest version only\n`);
    }
  });
  ...
  test('Documentation search within docs works', async ({ page }, testInfo) => {
    ...
    if (searchInput) {
      ...
      if (resultsFound) {
        fs.appendFileSync('test-logs.txt', `${testId} Documentation search returned results\n`);
      } else {
        fs.appendFileSync('test-logs.txt', `${testId} Search executed but results format unclear\n`);
      }
    } else {
      fs.appendFileSync('test-logs.txt', `${testId} No doc-specific search found - uses global search\n`);
    }
  });
  ...
  test('Documentation PDF/print versions are accessible', async ({ page }, testInfo) => {
    ...
    for (const selector of pdfPrintSelectors) {
      ...
      if (count > 0) {
        ...
        if (isVisible) {
          ...
          fs.appendFileSync('test-logs.txt', `${testId} Found PDF/Print option: ${text}\n`);
          pdfPrintFound = true;
          break;
        }
      }
    ...
    if (pdfPrintFound) {
      fs.appendFileSync('test-logs.txt', `${testId} PDF/Print version available\n`);
    } else {
      fs.appendFileSync('test-logs.txt', `${testId} No PDF/Print options found - may not be offered\n`);
    }
  });
});