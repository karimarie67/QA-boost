...
test.describe('Boost Download Tests', () => {
  ...
  test('Download links return valid HTTP status codes', async ({ page }, testInfo) => {
    ...
    fs.appendFileSync('test-logs.txt', `${testId} Found ${downloadLinks.length} download links\n`);
    ...
    for (const { href } of linksToCheck) {
      try {
        ...
        fs.appendFileSync('test-logs.txt', `${testId} Download link: ${href} - Status: ${status}\n`);
        
        // Should return 200 OK or 302 redirect
        expect(status).toBeGreaterThanOrEqual(200);
        expect(status).toBeLessThan(400);
      } catch (error) {
        fs.appendFileSync('test-logs.txt', `${testId} Failed to check ${href}: ${error.message}\n`);
      }
    }
  });
  ...
  test('Download file names are correct format', async ({ page }, testInfo) => {
    ...
    // Find download links
    const downloadLinks = await page.locator('a[href*="boost_1_"], a[href*=".tar.gz"], a[href*=".zip"]').all();
    fs.appendFileSync('test-logs.txt', `${testId} Found ${downloadLinks.length} download links\n`);
    ...
    for (const link of downloadLinks.slice(0, 5)) {
      ...
      // Verify filename format: boost_X_XX_X.tar.gz or similar
      const hasValidFormat = /boost_\d+_\d+_\d+\.(tar\.gz|zip|7z)/.test(href) || 
                            /\d+\.\d+\.\d+/.test(href);
      
      fs.appendFileSync('test-logs.txt', `${testId} Link: ${text?.trim()} -> ${href}, Valid format: ${hasValidFormat}\n`);
      
      if (href && href.includes('boost')) {
  ...
  test('Version selector displays available versions', async ({ page }, testInfo) => {
    ...
    if (versionDropdown) {
      ...
      fs.appendFileSync('test-logs.txt', `${testId} Available versions: ${optionTexts.join(', ')}\n`);
      expect(options.length).toBeGreaterThan(0);
    } else {
      fs.appendFileSync('test-logs.txt', `${testId} No version selector found - versions may be displayed differently\n`);
      
      // Look for version numbers in text
      const versionText = await page.locator('text=/1\\.8\\d+|version/i').count();
  ...
  test('Download page displays file sizes', async ({ page }, testInfo) => {
    ...
    for (const pattern of fileSizePatterns) {
      ...
      if (count > 0) {
        ...
        if (isVisible) {
          ...
          fs.appendFileSync('test-logs.txt', `${testId} Found file size: ${text}\n`);
          fileSizeFound = true;
          break;
        }
      }
    ...
    if (fileSizeFound) {
      fs.appendFileSync('test-logs.txt', `${testId} File sizes displayed on download page\n`);
    } else {
      fs.appendFileSync('test-logs.txt', `${testId} No file sizes found - may not be displayed on this page\n`);
    }
  });
});

test.describe('Boost Search Tests', () => {
  ...
  test('Search returns relevant results for common queries', async ({ page }, testInfo) => {
    ...
    const commonQueries = ['asio', 'filesystem', 'algorithm'];
    ...
    for (const query of commonQueries) {
      ...
      if (searchResults && resultCount > 0) {
        ...
        fs.appendFileSync('test-logs.txt', `${testId} Search for "${query}" returned ${resultCount} results\n`);
        ...
        fs.appendFileSync('test-logs.txt', `${testId} No results found for "${query}"\n`);
      }
  ...
  test('Search result pagination works correctly', async ({ page }, testInfo) => {
    ...
    await performSearch(page, testInfo, selectors, 'boost', testId);
    await page.waitForTimeout(2000);
    ...
    for (const selector of paginationSelectors) {
      ...
      if (count > 0) {
        ...
        if (isVisible) {
          fs.appendFileSync('test-logs.txt', `${testId} Pagination controls found\n`);
          ...
          if (nextExists && await nextButton.isVisible().catch(() => false)) {
            ...
            fs.appendFileSync('test-logs.txt', `${testId} Pagination click: URL changed = ${urlChanged}\n`);
          }
    ...
    if (!paginationFound) {
      fs.appendFileSync('test-logs.txt', `${testId} No pagination found - results may fit on one page\n`);
    }
  });
